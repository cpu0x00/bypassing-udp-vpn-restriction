...
API_TOKEN = 'YOUR_API_HERE' # the personal access token
...
def destroy_sshkey(): # destroys the ssh keys from the account and deletes them from the disk (on closing the script)
	...
	print('[*] deleted the vps-openvpn-client.ovpn')
...
def connect_thm():
	...
	system('openvpn thm.ovpn')


def connect_offsec():
	print('\n[*] connecting to offsec-playground vpn...')
	system('openvpn offsec_playground.ovpn')

def main():
	# parser = argparse.ArgumentParser()
	# parser.add_argument('--vpn', choices=['thm','offsec','cyberseclabs'])
	...
	try:
		...
		connect_openvpn()
		# if args.vpn == 'thm':
		# 	t1 = threading.Thread(target=connect_openvpn)	
		# 	t1.start()
		# 	t2 = threading.Thread(target=connect_thm)	
		# 	t2.start()
		# elif args.vpn == 'offses':
		# 	t1 = threading.Thread(target=connect_openvpn)	
		# 	t1.start()
		# 	t2 = threading.Thread(target=connect_offsec)	
		# 	t2.start()
		# else:

		destroy_thread1 = threading.Thread(target=destroy_droplet)
		destroy_thread1.start()
		destroy_thread2 = threading.Thread(target=destroy_sshkey)
		destroy_thread2.start()
